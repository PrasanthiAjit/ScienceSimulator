...
import { Chemical, SimulationResult } from '../types';
...
const chemicals: Chemical[] = [
  ...
  { formula: 'NaHCO3', name: 'Sodium Bicarbonate', state: 's', type: 'salt', color: '#ffffff' },
  { formula: 'KI', name: 'Potassium Iodide', state: 'aq', type: 'salt', color: '#ffffff' },
  { formula: 'Pb(NO3)2', name: 'Lead(II) Nitrate', state: 'aq', type: 'salt', color: '#ffffff' },
];
...
const ChemistryLab: React.FC = () => {
  ...
  const handleMix = async () => {
    ...
    setLoading(true);
    const data = await simulateChemicalReaction(reactantA.name, reactantB.name);
    setResult(data);
    setLoading(false);
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full p-4">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 col-span-1 overflow-y-auto">
        <h3 className="font-sans font-bold text-lg mb-4 flex items-center gap-2 text-slate-700">
            <Atom size={20} /> Chemical Inventory
        </h3>
        <div className="space-y-2">
            {chemicals.map((chem) => (
                <div key={chem.formula} className="flex items-center justify-between p-3 border rounded-lg hover:bg-slate-50 transition-colors">
                    <div>
                        <span className="font-mono font-bold text-slate-800 block">{chem.formula}</span>
                        <span className="text-xs text-slate-500">{chem.name}</span>
                    </div>
                    <div className="flex gap-2">
                        <button 
                            onClick={() => setReactantA(chem)}
                            className={`px-3 py-1 text-xs font-bold rounded ${reactantA?.formula === chem.formula ? 'bg-lab-blue text-white' : 'bg-slate-200 text-slate-600'}`}
                        >
                            A
                        </button>
                        <button 
                            onClick={() => setReactantB(chem)}
                            className={`px-3 py-1 text-xs font-bold rounded ${reactantB?.formula === chem.formula ? 'bg-lab-indigo text-white' : 'bg-slate-200 text-slate-600'}`}
                        >
                            B
                        </button>
                    </div>
        ...
        <div className="flex-grow bg-slate-900 rounded-xl shadow-inner p-8 relative flex items-center justify-center overflow-hidden">
            ...
            <div className="flex flex-col items-center gap-4 transition-all duration-500 transform translate-y-0 z-10">
                ...
                <div className="text-center">
                    ...
                    <span className="text-slate-400 text-sm">Reaction Chamber</span>
                </div>
            </div>
            ...
            {result && !loading && (
                <div className="absolute bottom-4 left-4 right-4 bg-white/95 backdrop-blur rounded-lg p-4 shadow-xl border-l-4 border-lab-teal animate-fade-in">
                    <h4 className="font-bold text-lab-teal mb-1">Reaction Analysis</h4>
                    <p className="font-mono text-lg text-slate-800 mb-2">{result.balancedEquation}</p>
        ...
        <div className="h-24 bg-white rounded-xl shadow-sm border border-slate-200 flex items-center justify-between px-8">
            <div className="flex items-center gap-4">
                <div className="text-right">
                    <span className="block text-xs text-slate-500">Reactant A</span>
                    <span className="font-bold text-lab-blue">{reactantA?.name || 'Select'}</span>
                </div>
                <div className="text-xl text-slate-300">+</div>
                <div>
                    <span className="block text-xs text-slate-500">Reactant B</span>
